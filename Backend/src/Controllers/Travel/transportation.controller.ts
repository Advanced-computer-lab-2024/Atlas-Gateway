...
export const booktransportation = async (req: Request, res: Response) => {
	try {
		...
		if (transportation.availability > transportation.numberOfBookings) {
			...
			const tourist = await addBookedTransportation(
				tId,
				transportation.id,
			);
			...
			if (!tourist) {
				return res
					.status(500)
					.json({ message: "Error booking Transportation" });
			}
...
export const bookTransportation = async (req: Request, res: Response) => {
	try {
		...
		const addBookingResult = addBookedTransportation(
			tourist.id,
			transportation.id,
		);
...
export const cancelBookingTransportation = async (
	...
	try {
		...
		const cancelBookingResult = cancelBookingTransportation(
			transportation.id,
			tourist.id,
		);

		const removeBookingResult = cancelTransportation(
			tourist.id,
			transportation.id,
		);
